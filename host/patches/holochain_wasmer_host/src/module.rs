...
impl ModuleCache {
    ...
    fn get_from_filesystem(&self, key: CacheKey) -> Result<Option<Bytes>, wasmer::RuntimeError> {
        // ARM64 libunwind bug: wasmer::RuntimeError::user() calls Backtrace::new_unresolved()
        // which invokes _Unwind_Backtrace and segfaults on Apple Silicon.  Treat every IO
        // error here as a clean cache miss so we never enter that code path.
