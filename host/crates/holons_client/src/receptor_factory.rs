...
use super::{
    receptor_cache::{ReceptorCache, ReceptorKey}
};
use core_types::HolonError;
use deprecated_holochain_receptor::{HolochainReceptor};
use client_shared_types::{base_receptor::{BaseReceptor, ReceptorType}, holon_space::SpaceInfo};
use std::sync::Arc;
...
impl ReceptorFactory {
    ...
    async fn create_receptor_from_base(&self, base: BaseReceptor) -> Result<Arc<Receptor>, Box<dyn std::error::Error>> {
        match base.receptor_type {
            ReceptorType::Local => { 
                Err(Box::new(HolonError::NotImplemented(
            ...
            ReceptorType::LocalRecovery => {
                ...
                Ok(Arc::new(Receptor::LocalRecovery(
                    LocalRecoveryReceptor::new(base)?,
                )))
            }
    ...
    pub fn get_receptors_by_type(&self, receptor_type: &ReceptorType) -> Result<Vec<Arc<Receptor>>, HolonError> {
    ...
    pub fn get_default_receptor_by_type(&self, receptor_type: &ReceptorType) -> Result<Arc<Receptor>, HolonError> {
        self
            .get_receptors_by_type(receptor_type)?
            .first()
            .cloned()
            .ok_or_else(|| HolonError::HolonNotFound(format!("No default receptor found for type: {:?}", receptor_type)))
    }
    ...
    pub fn get_receptor_by_id(&self, receptor_id: &String) -> Result<Arc<Receptor>, HolonError> {
        ...
        receptors
            .first()
            .cloned()
            .ok_or_else(|| HolonError::HolonNotFound(format!("No receptors found for id: {:?}", receptor_id)))
    }
    ...
    pub async fn all_spaces_by_type(&self, rec_type: &ReceptorType) -> Result<SpaceInfo, HolonError> {
