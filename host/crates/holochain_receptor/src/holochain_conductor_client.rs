            ...
            app_ws_guard
                ...
                .ok_or_else(|| {
                    HolonError::FailedToBorrow(
                        "Service is not yet initialized with AppSocket.".into(),
                    )
                })?
                .clone()
        ...
        match app_info_response {
            ...
            Err(e) => {
                ...
                Err(HolonError::NotImplemented(format!(
                    "Failed to get AppInfo from conductor: {:?}",
                    e
                )))
            }
        }
            ...
            match cell_info {
                CellInfo::Provisioned(provisioned_cell) => {
                    let sprops = HolonSpace {
                        ...
                        metadata: Some(to_bytebuf(
                            provisioned_cell.dna_modifiers.properties.clone(),
                        )),
                        enabled: true,
                    };
