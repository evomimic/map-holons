...
pub async fn spawn_listen_to_space_task(space: SpaceId, admin_port: u16) -> crate::Result<()> {
    let admin_ws =
        AdminWebsocket::connect(format!("localhost:{}", admin_port),None).await.map_err(|err| {
...
async fn wait_until_admin_ws_is_available(admin_port: u16) -> crate::Result<AdminWebsocket> {
    ...
    loop {
        let connect_result = AdminWebsocket::connect(format!("localhost:{}", admin_port),None).await;
