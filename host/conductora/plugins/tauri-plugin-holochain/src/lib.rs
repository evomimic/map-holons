...
impl<R: Runtime> HolochainPlugin<R> {
    ...
    pub async fn web_happ_window_builder(
        ...
        let app_websocket_auth =
            self.holochain_runtime.get_app_websocket_auth(&app_id, allowed_origins).await?;

        let token_vector: Vec<String> =
            app_websocket_auth.token.iter().map(|n| n.to_string()).collect();
    ...
    pub async fn app_websocket(&self, app_id: InstalledAppId) -> crate::Result<AppWebsocket> {
        ...
        let app_ws =
            self.holochain_runtime.app_websocket(app_id, AllowedOrigins::Origins(origins)).await?;
        Ok(app_ws)
    }
    ...
    pub async fn install_web_app(
        ...
        let app_info = self
            .holochain_runtime
            .install_web_app(app_id.clone(), web_app_bundle, roles_settings, agent, network_seed)
            .await?;
    ...
    pub async fn install_app(
        ...
        let app_info = self
            .holochain_runtime
            .install_app(app_id.clone(), app_bundle, roles_settings, agent, network_seed)
            .await?;
    ...
    pub async fn update_web_app(
        ...
        self.holochain_runtime.update_web_app(app_id.clone(), web_app_bundle).await?;
...
fn plugin_builder<R: Runtime>() -> Builder<R> {
    Builder::new("holochain")
        ...
        .register_uri_scheme_protocol("happ", |context, request| {
            tracing::info!("Received request {}", request.uri().to_string());
            ...
            tauri::async_runtime::block_on(async move {
                ...
                let uri_components: Vec<String> =
                    uri_without_querystring.split("/").map(|s| s.to_string()).collect();
                let lowercase_app_id =
                    uri_components.get(0).expect("Malformed request: not enough items 3");
                ...
                .await
                {
                    Ok(Some((asset, mime_type))) => {
                        tracing::info!("Got asset for app with id: {}", lowercase_app_id);
                        let mut response =
                        ...
                        return response.body(asset).expect("Failed to build response with asset");
                    }
                ...
                if tauri::is_dev() {
                    if let Ok(h) = app.holochain() {
                        if let Err(err) = delete_hc_live_file(h.holochain_runtime.admin_port) {
                            tracing::error!("Failed to delete hc live file: {err:?}");
                        }
                    }
                }
            }
    ...
    plugin_builder()
        .setup(|app, _api| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                {
                    tracing::error!("Failed to launch holochain: {err:?}");
...
async fn launch_and_setup_holochain<R: Runtime>(
    ...
    // tracing::info!("Starting http server at port {http_server_port:?}");
    ...
    let p = HolochainPlugin::<R> { app_handle: app_handle.clone(), holochain_runtime };
