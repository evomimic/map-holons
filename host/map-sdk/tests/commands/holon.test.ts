...
import {
  ...
  readAvailableDances,
  readEffectiveCardinality,
  readPropertyIsArray,
  readRelationshipIsOrdered,
...
const holonCases: HolonCase<unknown>[] = [
  ...
  {
    name: 'readAvailableDances',
    run: () => readAvailableDances(txId, target),
    action: { Read: 'GetAvailableDances' },
    okResult: { Collection: collection },
    expected: collection,
    wrongResult: { Value: stringValue },
  },
  {
    name: 'readEffectiveCardinality',
    run: () => readEffectiveCardinality(txId, target),
    action: { Read: 'GetEffectiveCardinality' },
    okResult: { EffectiveCardinality: { minimum: 0, maximum: null } },
    expected: { minimum: 0, maximum: null },
    wrongResult: { Value: stringValue },
  },
  {
    name: 'readPropertyIsArray',
    run: () => readPropertyIsArray(txId, target),
    action: { Read: 'GetPropertyIsArray' },
    okResult: { Value: { BooleanValue: true } },
    expected: true,
    wrongResult: { Collection: collection },
  },  {
    name: 'readRelationshipIsOrdered',
    run: () => readRelationshipIsOrdered(txId, target),
    action: { Read: 'GetRelationshipIsOrdered' },
    okResult: { Value: { BooleanValue: true } },
    expected: true,
    wrongResult: { Collection: collection },
  },
