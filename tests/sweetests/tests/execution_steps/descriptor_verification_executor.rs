...
use holons_test::harness::helpers::{
    ...
    OPERATOR_CATEGORY_ORDERING_KEY, PERSON_DESCRIPTOR_KEY, PERSON_TO_BOOK_RELATIONSHIP_INVERSE_KEY,
    PERSON_TO_BOOK_REL_INVERSE, SCHEMA_TYPE_KEY, STAGE_NEW_VERSION_BOOK_KEY,
...
pub async fn execute_verify_core_schema_descriptors(state: &mut TestExecutionState) {
    ...
    assert_eq!(
        holon_id_projection_descriptor.header().type_name().expect("HolonId.Projection type_name"),
        MapString("HolonId".to_string())
    );
    assert_contains(&related_holon_keys(&holon_id_projection, "Extends"), "Projection.HolonType");
    assert_contains(
...
pub async fn execute_verify_book_person_descriptors(state: &mut TestExecutionState) {
    ...
    assert!(title.is_required().expect("Title requiredness"));
    assert_eq!(
        title
            .value_type()
