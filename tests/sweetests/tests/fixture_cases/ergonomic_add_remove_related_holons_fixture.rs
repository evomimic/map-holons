...
pub fn ergonomic_add_remove_related_holons_fixture() -> Result<DancesTestCase, HolonError> {
    ...
    let mut book_expected_content = EssentialHolonContent::new(
        book_properties,
        EssentialRelationshipMap::default(),
        Some(book_key.clone()),
        Vec::new(),
    );
    book_expected_content.relationships.add_related_holons(
        &*fixture_context,
    ...
    // Expected
    book_expected_content.relationships.remove_related_holons(
        &*fixture_context,
        &PUBLISHED_BY.to_relationship_name(), // str
        vec![HolonReference::Transient(publisher_transient_reference.clone())],
    )?;
    ...
    let mut staged_person_1_expected_content = EssentialHolonContent::new(
        person_1_properties.clone(),
        EssentialRelationshipMap::default(),
        Some(person_1_key.clone()),
        Vec::new(),
    );
    staged_person_1_expected_content.relationships.add_related_holons(
        &*fixture_context,
    ...
    // Assert
    assert_eq!(
        staged_person_1_expected_content,
        person_1_staged_reference.essential_content(&*fixture_context)?
    );
    //
    // -- //
    ...
    // Expected
    staged_person_1_expected_content.relationships.remove_related_holons(
        &*fixture_context,
