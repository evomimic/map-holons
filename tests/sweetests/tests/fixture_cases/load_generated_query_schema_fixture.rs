...
/// Proves that Query loads after Core and that its Dance adapter loads only
/// after Core, Dance, and Query have committed.
pub fn load_generated_query_schema_fixture() -> Result<DancesTestCase, HolonError> {
    let TestCaseInit { mut test_case, fixture_context, fixture_holons, .. } = TestCaseInit::new(
        ...
        "Load Core, Dance, Query, then the generated Query Dance Adapter package in separate transactions",
    );
