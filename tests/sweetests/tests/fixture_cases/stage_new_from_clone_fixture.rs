...
use holons_core::{HolonsContextBehavior, dances::ResponseStatusCode, new_holon, WritableHolon};
use holons_test::{fixture_holons::FixtureHolons, test_case::DancesTestCase};
use type_names::ToPropertyName;

use crate::{fixture_cases::setup_book_and_authors_fixture::*, helpers::BOOK_KEY};
...
pub fn stage_new_from_clone_fixture(
    fixture_context: &dyn HolonsContextBehavior,
) -> Result<DancesTestCase, HolonError> {
    ...
    // // Optional: assert empty DB to start (derived from current expectations)
    // test_case.add_ensure_database_count_for_fixture(&fixture_holons)?;

    // ── PHASE A — Clone FROM a fresh TRANSIENT ────────────────────────────────────
    const TRANSIENT_SOURCE_KEY: &str = "book:transient-source";
    let transient_source =
        new_holon(fixture_context, MapString::from(TRANSIENT_SOURCE_KEY))?
    ...
    test_case.add_with_properties_step(
        clone_from_transient_staged.clone(),
        p,
        ResponseStatusCode::Ok,
    )?;
    ...
    test_case.add_commit_step(fixture_context, ResponseStatusCode::OK)?;
    ...
    p3.insert("TITLE".into(), "Dune (Saved Clone)".into());
    p3.insert("EDITION".into(), MapInteger::from(3).into());
    test_case.add_with_properties_step(
