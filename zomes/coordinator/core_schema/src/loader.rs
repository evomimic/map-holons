...
use type_definers::descriptor_types_deprecated::{CoreSchemaName, Schema};
...
/// The load_core_schema function creates a new Schema Holon and populates it type_definers_zome for all the
...
pub fn load_core_schema(context: &dyn HolonsContextBehavior) -> Result<CommitResponse, HolonError> {
    ...
    let schema = Schema::new(
        ...
        MapString(
            "The foundational MAP type type_definers_zome for the L0 layer of the MAP Schema".to_string(),
        ),
    )?;
