...
#[derive(Debug, Clone)]
pub struct HolonCache(Cache<LocalId, Rc<RefCell<Holon>>>);
...
impl HolonCache {
    ...
    pub fn get(&self, key: &LocalId) -> Option<&Rc<RefCell<Holon>>> {
        self.0.get(key)
    }
    /// Inserts an item into the cache.
    pub fn insert(&mut self, key: LocalId, value: Rc<RefCell<Holon>>) {
        self.0.insert(key, value);
    }
}
impl Deref for HolonCache {
    type Target = Cache<LocalId, Rc<RefCell<Holon>>>;
