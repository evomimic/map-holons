...
pub fn define_type_descriptor() -> Holon {
    ...
    descriptor.with_property_value(PropertyName(MapString("type_name".to_string())), BaseValue::StringValue(MapString("TypeDescriptor".to_string())))
        .with_property_value(PropertyName(MapString("description".to_string())), BaseValue::StringValue(MapString(("A meta-descriptor that defines the properties, relationships and dances shared by all MAP type_definers_zome (including itself).".to_string()))))
