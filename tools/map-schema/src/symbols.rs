//! Derived symbol table for semantic schema models.
//!
//! The table is intentionally in-memory only. It provides stable IDs for one build of a
//! [`SemanticModel`], lookup indexes for emitters/tests, and diagnostics for duplicate symbols or
//! invalid references.
...
/// Stable symbol identity within one derived symbol table.
...
impl SymbolTable {
    ...
    /// Builds a symbol table, resolves references in the model, and returns diagnostics.
    pub fn from_model(model: &mut SemanticModel) -> (Self, Vec<Diagnostic>) {
        let mut table = Self::new();
        let mut diagnostics = Vec::new();

        for schema in &model.schemas {
            if let Err(diagnostic) = table.insert_schema(schema) {
                diagnostics.push(diagnostic);
            }
        }

        for descriptor in &model.descriptors {
            if let Err(diagnostic) = table.insert_descriptor(descriptor) {
                diagnostics.push(diagnostic);
            }
        }

        model.resolve_references(&table);
        diagnostics.extend(table.collect_reference_diagnostics(model));
        (table, diagnostics)
    }
    ...
    pub fn lookup_by_key(&self, key: &str) -> Option<&Symbol> {
        self.by_key.get(key).and_then(|id| self.lookup_by_id(*id))
    }
...
fn push_reference_diagnostic(
    ...
    let Some(symbol) = table.lookup_by_id(symbol_id) else {
        return;
    };
...
mod tests {
    ...
    use crate::{
        ...
        semantic::{push_reference, TdlDescriptorInput},
    };
    ...
    fn detects_wrong_descriptor_kind_when_expected_kind_is_known() {
        ...
        let (_, diagnostics) = SymbolTable::from_model(&mut model);
    ...
    fn semantic_comparison_ignores_origin_differences() {
        ...
        let (_, json_diagnostics) = SymbolTable::from_model(&mut json_model);
        let (_, tdl_diagnostics) = SymbolTable::from_model(&mut tdl_model);
