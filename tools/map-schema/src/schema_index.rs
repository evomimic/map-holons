//! Derived lookup indexes for the canonical schema IR.
//!
...
impl SymbolIndex {
    ...
    /// Looks up a symbol by an authored reference target, allowing suffix aliases such as
    /// `Schema.HolonType` to resolve to the underlying `Schema` symbol when appropriate.
