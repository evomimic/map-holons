...
use shared_types_holon::{HolonId, LocalId, ExternalId};

use crate::holon::Holon;
use crate::holon_error::HolonError;
use crate::holon_node::{
    create_path_to_holon_node, get_original_holon_node, get_holon_node_by_path, CreatePathInput, GetPathInput
};
...
pub fn create_local_path(target_holon_hash: LocalId, path_name:String, linktype:LinkTypes) -> Result<ActionHash, HolonError> {
...
pub fn get_holon_by_path(path_name: String, linktype:LinkTypes) -> Result<Option<Holon>, HolonError> {
    let path = Path::from(path_name);
    let link_type = linktype;
    let input = GetPathInput { path:path.clone(), link_type };
    let result = get_holon_node_by_path(input)
...
pub fn get_holon_by_id(holon_id: &HolonId) -> Result<Holon, HolonError> {
    if holon_id.is_local() {
        ...
        let external_id = match holon_id.external_id() {
            Some(id) => id,
            None => return Err(HolonError::InvalidHolonReference("HolonId is not External".to_string())),
        };
...
pub fn get_holon_by_external_id(external_id: &ExternalId) -> Result<Holon, HolonError> {
    return Err(HolonError::NotImplemented(format!("external_id: {} feature not built",external_id.space_id.0.to_string())));
}
