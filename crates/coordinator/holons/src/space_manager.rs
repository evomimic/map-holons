...
impl HolonSpaceManager {
    ...
    pub fn new_from_session(staged_holons: Vec<Rc<RefCell<Holon>>>, keyed_index: BTreeMap<MapString, usize>, space_holon_ref: Option<HolonReference> ) -> Self {
    ...
    pub fn ensure_local_holon_space(&mut self,context:&HolonsContext) -> Result<HolonReference, HolonError> {
        return match self.get_space_holon() {
            ...
            None => {
                ...
                let spaceholon = holon_service::get_holon_by_path("local_holon_space".to_string(), LinkTypes::LocalHolonSpace);
                match spaceholon {
                    Ok(spaceholon) => {
                        match spaceholon {
                            Some(spaceholon) => {
                                ...
                                let ref_spaceholon = HolonReference::smartreference_from_holon_id(holon_id);
                            ...
                            None => {
                                ...
                                uncommited_space_holon.with_name(&name)?.with_description(&description)?;
                                let _stage_result = self.stage_new_holon(uncommited_space_holon.clone())?;
                                // COMMIT
                                ...
                                if commit_response.is_complete() {
                                    ...
                                    let create_local_path = holon_service::create_local_path(local_id,"local_holon_space".to_string(), LinkTypes::LocalHolonSpace);
                                    match create_local_path {
                                        Ok(_) => {
                                            let space_reference = HolonReference::smartreference_from_holon_id(holon_id);
                                            self.set_space_holon_ref(space_reference.clone());
                                            return Ok(space_reference)
                                        }
                                        Err(_) => {
                                            return Err(HolonError::CommitFailure("Unable to crate path to the LocalHolonSpace".to_string()));
                                        }
                                    }
                                } else {
