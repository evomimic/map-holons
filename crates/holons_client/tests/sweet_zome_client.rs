...
pub trait ZomeClient: Sized {
 async fn install(app_name:&str, happ_url:Option<&str>) -> Result<Self, ConductorApiError>;
...
async fn mytest() {
    let app: AppInstallation = AppInstallation::install("map_holons",Some("../../workdir/map_holons.happ")).await.unwrap();
    ...
    app.zomecall(cell_id,"holons","commit", 
...
impl ZomeClient for AppInstallation {
    ...
    async fn zomecall(self, cell_id:CellId, zome_name:&str, fn_name:&str, request:DanceRequest) -> Result<CommitResponse, ConductorApiError> {
        ...
        let commit_response = CommitResponse { 
            ...
            commits_attempted: MapInteger(0),
