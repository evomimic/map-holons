...
use holons_core::{
    as_json, AccessType, CommitRequestStatus, CommitResponse, Holon, HolonCollection, HolonError,
    HolonReadable, HolonState, HolonsContextBehavior, RelationshipName,
};
...
impl CommitService {
    ...
    pub fn commit(
        ...
        // FIRST PASS: Commit Staged Holons
        {
            ...
            for rc_holon in staged_holons {
                ...
                let outcome = self.commit_holon(rc_holon);
                match outcome {
        ...
        //  SECOND PASS: Commit relationships
        {
            ...
            for rc_holon in staged_holons {
                ...
                let outcome = self.commit_relationships(context, rc_holon);
    ...
    fn commit_holon(&self, rc_holon: &Rc<RefCell<Holon>>) -> Result<Holon, HolonError> {
        let holon = rc_holon.borrow();
        debug!(
            "Entered Holon::commit for holon with key {:#?} in {:#?} state",
            holon.get_key()?.unwrap_or_else(|| MapString("<None>".to_string())).0,
            holon.state
        );
        match holon.state {
            HolonState::New => {
                ...
                let result = create_holon_node(holon.clone().into_node());

                match result {
                    Ok(record) => {
                        holon.state = HolonState::Saved;
                        holon.saved_node = Option::from(record);

                        Ok(holon.clone())
                    }
                    Err(error) => {
                        let holon_error = HolonError::from(error);
                        holon.errors.push(holon_error.clone());
                        Err(holon_error)
                    }
                }
            }
            ...
            HolonState::Changed => {
                ...
                if let Some(ref node) = holon.saved_node {
                    let original_holon_node_hash = match holon.get_original_id()? {
                        Some(id) => Ok(id.0),
            ...
            _ => {
                ...
                debug!("Skipping commit for holon in {:#?} state", holon.state);
            ...
            HolonState::Saved => {
                match holon.saved_node.clone() {
                    Some(record) => {
                        ...
                        for (name, holon_collection) in holon.relationship_map.0.clone() {
                            debug!("COMMITTING {:#?} relationship", name.0 .0.clone());
                            self.commit_relationship(
                                context,
    ...
    fn commit_relationship(
        ...
        self.save_smartlinks_for_collection(context, source_id.clone(), name.clone(), collection)?;
