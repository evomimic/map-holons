...
// Isolated subject and outcome-installation tests retain the pre-aggregate fixture.
// Public Commit assessment is exercised through readiness and Sweettests.
#[cfg(test)]
pub(crate) mod subject_gate_tests {
    use super::*;
    use crate::handlers::finding;
    use crate::validators::{resolve_holon_descriptor, validate_described_holon};
    use crate::{HolonValidationContext, HolonValidationSubject, ValidationCollector};
    use core_types::{CommitValidationViolation, CommitValidationViolationKind};
    use holons_core::core_shared_objects::transactions::TransactionContext;
    use holons_core::{
        Descriptor, DescriptorReader, HolonDescriptor, HolonReference, ProspectiveIdentity,
        ReadableHolon,
    };
    use std::sync::Arc;

    pub(crate) fn validate_subject_candidates(
    ...
    fn assess_competition(
        ...
        for candidate in candidates {
            ...
            if !contract_content_available(context, &subject, reader, &mut collector)? {
                continue;
            }
            let Some(descriptor) = resolve_holon_descriptor(
    ...
    /// Legacy subject-gate fixtures preflight only definition dependencies before dispatch.
    /// The production gate traverses these commitments through `commitment_order`.
    fn contract_content_available(
        context: &Arc<TransactionContext>,
        subject: &HolonReference,
        reader: &holons_core::ProspectiveDescriptorReader,
        collector: &mut ValidationCollector,
    ) -> Result<bool, HolonError> {
        use crate::assessment_support::{recover, targets};
