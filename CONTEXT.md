...
- R7 acceptance is bounded to canonical Core Schema generated JSON plus the existing book/person inverse fixture. Arbitrary holon-shaped import round-tripping is a later source-tooling issue, not part of R7.
- R7 mirrors the R6 source boundary in the decompile direction: JSON-to-TDL is a syntactic transformation over loader facts and must not resolve descriptors, inspect contracts, materialize defaults, or perform descriptor-informed validation. Fidelity compares deterministic projections of authored LoaderRefRep facts, not resolved descriptor semantics.
- R7 round-trip fidelity uses canonical normalization rather than exact source spelling. JSON parsing may accept compatible legacy shapes, but `JSON -> TDL -> JSON` should converge to R6 canonical JSON; repeated round trips should then be stable.
- R7 canonical TDL should prefer concise descriptor syntax when the authored loader facts mechanically support it, with literal-safe fallback for any residue that concise syntax cannot preserve. The emitter must not choose a concise form by resolving descriptors or inferring contracts.
- R7 may keep its LoaderRefRep projection/signature local to `tools/map-schema` as temporary source-tooling support, mirroring the practical R6 implementation. It must remain descriptor-semantic-free, transaction-free, and deterministic; R8 remains responsible for deleting or moving transitional projection structures.
- R7 excludes `meta.load_with` from canonical fidelity because it is import orchestration metadata rather than authored holon content. As a legacy compatibility bridge, JSON decompile may use `meta.load_with` to infer missing schema `depends_on` clauses only when the schema holon lacks explicit `DependsOn` facts.
- R7 keeps Core Schema decompile output scratch-only by default, such as `generated/tdl-decompiled`, until fidelity is proven and promotion is explicit. Ordinary decompile commands must not overwrite the authoritative `schema-src` corpus.
- R7 fidelity compares canonical ordering rather than original source order for properties and relationship entries, using the R6 canonical order rules. Relationship target order remains authored and fidelity-significant.
- R7 preserves file partitioning for descriptor and instance holons in the acceptance corpus, but normalizes schema holon ownership to the R6 owner-file rule: each schema holon is emitted once, preferring the root file, while other files may use the same `schema` declaration only as packaging context.
- In R7 decompile, non-owner files for a shared schema emit only the minimal `schema <SchemaKey>` packaging declaration. They do not repeat schema `depends_on`, header, properties, or relationships.
- R7 decompile preserves authored relationship-pair direction. Declared relationships may collapse explicit `HasInverse` facts into declared-side shorthand when unambiguous; inverse relationships may collapse explicit `InverseOf` facts only when present. The emitter must never infer the opposite paired fact from source/target reversal or corpus adjacency.
- R7 decompile preserves authored enum-variant direction. Inline variants may be emitted from explicit enum-side `Variants` targets when all targeted variant holons are present and inline rendering is lossless; variant membership must not be inferred from key shape alone, and variant-side `VariantOf` is preserved only when explicitly authored.
- R7 fidelity comparison uses deterministic LoaderRefRep signatures as the acceptance gate, with canonical JSON diffs available as debugging and stability artifacts. Signatures include file partition, holon key, top-level type, canonical properties, canonical relationships, and relationship target order; they exclude `meta`, formatting, source spelling, and provenance.
- R7 should expose a narrow user-facing round-trip command for the acceptance workflow, such as `map-schema roundtrip-json <json-input> --tdl-out <dir> --json-out <dir>`, plus a Core Schema npm alias. The command writes scratch artifacts, compares signatures, reports pass/fail, and never modifies `schema-src`.
- R7 vertically replaces the public `map-schema decompile` behavior. Legacy `SemanticModel` decompile helpers may remain temporarily during implementation, but the default decompile command must use the R7 syntactic loader-fact path once R7 lands.
- R8 is bounded to retiring transitional source-tooling IR inside `tools/map-schema`. It should remove obsolete map-schema adapters and old paths that still route through `SemanticModel`, `schema_ir`, `loader_ir`, or `schema_to_loader_ir`, but it should not become a repository-wide removal of shared semantic crates or runtime descriptor infrastructure.
- R8 removes the old `map-schema symbols` command instead of rebuilding it over loader-fact projection. A replacement source-symbol or descriptor-index command should be a later feature with an explicit consumer.
- R8 should physically delete retired `tools/map-schema` IR modules once their callers are removed, rather than leaving dead public modules for reference. Any narrow test-only helper that survives should be local to the test that needs it and named as transitional.
- R8 acceptance should pivot fully to generated canonical JSON plus narrow compatibility fixtures. Broad legacy host-import JSON corpus tests must not be treated as authoritative because that corpus may contain source drift such as legacy relationship names.
- R8 should delete now-unused `tools/map-schema` support modules such as old symbol/index helpers as part of the drawdown. Active diagnostics support may remain if it is still used by the current `check` path.
- R8 should delete the old semantic TDL compilation path as well as the old JSON decompile path. The surviving map-schema surfaces are the active R6/R7 compile, check, decompile, and roundtrip commands.
- R8 should delete the R0 test-private Transitional Corpus Scanner once any still-needed invariants are covered by real parser/lowered-output assertions. R8 must not leave a second TDL parser in tests.
- R8 should replace active parser use of `map_schema_semantic` literal containers with tool-local source syntax structs when the change is mechanical. It should not turn that into a broader shared semantic-crate removal.
- R8 should remove the broad crate-level dead-code allowance added during R7 and leave `tools/map-schema` warning-clean. Only narrow, intentional field-level allowances should remain where serde or test-only shapes require them.
- R8 should keep the R6/R7 user-facing command surface and scratch directories stable: `compile`, `check`, `decompile`, `roundtrip-json`, the existing Core Schema npm aliases, `generated/tdl-decompiled`, and `generated/json-roundtrip`. The only intended command removal is `symbols`.
- R8 should remove dependencies from `tools/map-schema/Cargo.toml` once they are unused by active source tooling, including `map_schema_semantic` if the local cleanup eliminates the last map-schema reference. This dependency cleanup is limited to the map-schema tool crate.
